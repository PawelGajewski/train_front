...
const ChartView = ({ id, width, height }) => {
  ...
  const addSquat = async () => {
    const data = new FormData();
    data.append("ownerID", localStorage.getItem("ID"));
    data.append("weight", newWeight);
    axios.post("http://localhost:8000/squat", data, {}).then((res) => {});
  };

  useEffect(() => {
    axios
      .get("http://127.0.0.1:8000/squat?search=" + localStorage.getItem("ID"))
      .then((res) => {
        setWeight(res.data);
      })
      .catch((err) => {
  ...
  return (
    <div>
      ...
      <Modal open={modal} show="True" onClose={closeModal}>
        <form>
          <p></p>
          <h3>Podaj wynik przysiadu</h3>
          <input
            type="text"
            className="form-control"
            placeholder="wynuk (w cm)"
            name="weight"
            value={newWeight}
          ...
          />
          <p></p>
          <button className="btn btn-primary" onClick={addSquat}>
            Zapisz
          </button>
        </form>
      </Modal>
